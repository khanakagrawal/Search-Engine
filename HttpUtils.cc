...
bool IsPathSafe(const string &root_dir, const string &test_file) {
  ...
  // STEP 1
  if (root_dir.empty() || test_file.empty()) {
    return false;
  }
  ...
  // get the absolute path for the directory and the file
  if (realpath(root_dir.c_str(), dir_path) == NULL ||
      realpath(test_file.c_str(), file_path) == NULL) {
    return false;
  }

  size_t dir_path_len = strlen(dir_path);

  // make sure the length of the file path is more than that of the directory
  if (dir_path_len > strlen(file_path)) {
    return false;
  }

  // seperate the dir path and file name from the file path
  string ck_file_path = string(file_path).substr(0, dir_path_len);
  string ck_file_name = string(file_path).substr(dir_path_len,
                                                 strlen(file_path));

  // make sure the dir path from file path is the same as the dir path
  if (ck_file_path != string(dir_path)) {
    return false;
  }

  // make sure the char after dir path is / for the next part or is the end
  if (file_path[dir_path_len] != '/' && file_path[dir_path_len] != '\0') {
    return false;
  }
...
string EscapeHtml(const string &from) {
  ...
  for (uint32_t i = 0; i < ret.length(); i++) {
    if (ret[i] == '"') {
      ...
      // make sure this & isn't the start of an escape code
      string sub_long = ret.substr(i+1, 5);
      string sub_short = ret.substr(i+1, 3);
      if (sub_long == "quot;" || sub_long == "apos;" ||
          sub_short == "lt;" || sub_short == "gt;") {
        continue;
      }

      // if not, replace it with & escape code
      ret.replace(i, 1, "&amp;");
    }
  }
  return ret;
