...
// Helper, Creates a linked list iterator and stops if the given key is found.
// Params:
//   LinkedList *chain: A pointer to the linked list to look through
//   HTKey_t key: the key to look for
// Returns:
//   LLIterator* : a pointer to the iterator, stopped at the found node or NULL
//               if not found.
static LLIterator* Find_Node(LinkedList *chain, HTKey_t key);
...
bool HashTable_Insert(HashTable *table,
  ...
  // we can increase since remove will decrease if necessary
  table->num_elements++;

  if (HashTable_Remove(table, newkeyvalue.key, oldkeyvalue)) {
...
bool HashTable_Find(HashTable *table,
  ...
  LLIterator *iter = Find_Node(table->buckets[bucket], key);

  // this means node was not found
  if (iter == NULL) {
    return false;
  }

  HTKeyValue_t *node;
  LLIterator_Get(iter, (LLPayload_t *) &node);
  keyvalue->key = node->key;
...
bool HashTable_Remove(HashTable *table,
  ...
  LinkedList *temp = table->buckets[bucket];
  LLIterator *iter = Find_Node(temp, key);

  // nothing to remove since node was not found
  if (iter == NULL) {
    return false;
  }
  ...
  // storing the found key and value before removing it
  keyvalue->key = node->key;
  keyvalue->value = node->value;
...
static LLIterator* Find_Node(LinkedList *chain, HTKey_t key) {
  if (chain == NULL || LinkedList_NumElements(chain) == 0) {
    return NULL;
  }
  ...
  while (LLIterator_IsValid(iterator)) {
    LLIterator_Get(iterator, (LLPayload_t *) &node);
    if (node->key == key) {
      return iterator;  // Key found
    }
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  // since the current iterator is not valid anyway
  free(iter->bucket_it);
  iter->bucket_idx++;

  while (iter->bucket_idx  < iter->ht->num_buckets) {
    // only allocate new iterator if the list has at least one element
    if (LinkedList_NumElements(iter->ht->buckets[iter->bucket_idx]) == 0) {
      iter->bucket_it = NULL;
    } else {
      int idx = iter->bucket_idx;
      iter->bucket_it = LLIterator_Allocate(iter->ht->buckets[idx]);
      return true;
    }
    iter->bucket_idx++;
  }
...
bool HTIterator_Get(HTIterator *iter, HTKeyValue_t *keyvalue) {
  ...
  HTKeyValue_t *node;
  LLIterator_Get(iter->bucket_it, (LLPayload_t *) &node);
  keyvalue->key = node->key;
  keyvalue->value = node->value;
