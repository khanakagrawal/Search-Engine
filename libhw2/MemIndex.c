...
LinkedList* MemIndex_Search(MemIndex* index, char* query[], int query_len) {
  ...
  bool found = HashTable_Find(index, key, &kv);
  if (!found) {
    return NULL;
  }

  // found the wordpostings HT, start iterating through it
  wp = kv.value;
  HTIterator *iter = HTIterator_Allocate(wp->postings);
  ret_list = LinkedList_Allocate();

  while(HTIterator_IsValid(iter)) {
    // get the current doc_id and it's corresponding postings 
    HTKeyValue_t doc_to_postings;
    bool get = HTIterator_Get(iter, &doc_to_postings);
    ...
    // put it in a SearchResult
    SearchResult *sr = (SearchResult *) malloc(sizeof(SearchResult));
    sr->doc_id = doc_to_postings.key;
    sr->rank = LinkedList_NumElements(doc_to_postings.value);
  ...
  for (i = 1; i < query_len; i++) {
    ...
    for (j = 0; j < num_docs; j++) {
      ...
      // get the current word postings and its hashtable
      wp = kv.value;
      HTKeyValue_t postings;
      bool found = HashTable_Find(wp->postings, sr->doc_id, &postings); 

      // if it is found, add to the rank, if not, remove current SearchResult
      if (found) {
        sr->rank +=  LinkedList_NumElements(postings.value);
