...
static void HandleDir(char* dir_path, DIR* d, DocTable** doc_table,
  ...
  while (dirent != NULL) {
    ...
    char* dot = ".";
    char* dot2 = "..";
    if (strcmp(dirent->d_name, dot) == 0 || strcmp(dirent->d_name, dot2) == 0) {
      dirent = readdir(d);
      continue;
    }
...
static void HandleFile(char* file_path, DocTable** doc_table,
  ...
  // of the file.
  char* file_string = ReadFileToString(file_path, &file_len);
  if (file_string == NULL) {
    return;
  }
  tab = ParseIntoWordPositionsTable(file_string);
  ...
  while (HTIterator_IsValid(it)) {
    ...
    bool removed = HTIterator_Remove(it, &kv);
    if (removed) {
      wp = kv.value;
