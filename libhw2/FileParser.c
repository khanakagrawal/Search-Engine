...
char* ReadFileToString(const char* file_name, int* size) {
  ...
  if (result != 0) {
    fprintf(stderr, "stat error open %s file\n", file_name);
  ...
  buf = (char *) malloc(file_stat.st_size + sizeof('\0'));
...
static void InsertContent(HashTable* tab, char* content) {
  ...
  while (*cur_ptr != '\0') {
    if (isalpha(*cur_ptr)) {
        if (word_start == NULL) {  // start of a new word
            word_start = cur_ptr;
            num_words++;
        }
        *cur_ptr = tolower(*cur_ptr);
    } else {
        if (word_start != NULL) {  // end of a word
            *cur_ptr = '\0';
            u_int32_t offset = word_start - content;
            AddWordPosition(tab, word_start, (DocPositionOffset_t) offset);
            word_start = NULL;
        }
    }
    cur_ptr++;
  }
  // if the last word reaches the end of the string
  if (word_start != NULL) {
      u_int32_t offset = word_start - content;
      AddWordPosition(tab, word_start, (DocPositionOffset_t) offset);
  }
