...
static void HttpServer_ThrFn(ThreadPool::Task *t) {
  ...
  std::cout << "successfully in httpserver_thrfn" << std::endl;
  ...
  while (!done) {
    ...
    // check if the client wants to close the connection, close it if so
...
static HttpResponse ProcessFileRequest(const string &uri,
  ...
  // parse and extract file path from uri
  URLParser url_parser;
  url_parser.Parse(uri);
  file_name = url_parser.path();

  // remove the /static/ part from file path
  file_name.replace(0, 8, "../projdocs/");
  ...
  if (!fr.ReadFile(&contents)) {
    // couldn't find the file, return an HTTP 404 error.
  ...
  // put the file contents into the response
  ret.AppendToBody(contents);

  // set Content-type
  size_t index = file_name.find_last_of(".");
  string suffix = file_name.substr(index, file_name.length() - 1);
  ...
  if (suffix == ".html" || suffix == ".htm") {
    ...
    ret.set_content_type("image/x-icon");
  } else if (suffix == ".mp3") {
    ret.set_content_type("audio/mpeg");
  } else if (suffix == ".mp4") {
    ret.set_content_type("video/mp4");
  } else if (suffix == ".ogg") {
    ret.set_content_type("application/ogg");
  } else {
...
static HttpResponse ProcessQueryRequest(const string &uri,
  ...
  URLParser url_parser;
  url_parser.Parse(uri);
  std::map<std::string, std::string> args = url_parser.args();
  ...
  // if no search terms, just display the logo + search bar
  if (query.empty()) {
    return ret;
  }

  // otherwise show query results too
  // seperate the query words
  vector<string> words;
  boost::split(words, query, boost::is_any_of(" "), boost::token_compress_on);

  // instantiate query processor and process query
  hw3::QueryProcessor qp(indices);
  vector<hw3::QueryProcessor::QueryResult> results = qp.ProcessQuery(words);

  if (results.empty()) {
    // no results found
    ret.AppendToBody("<p>No results found for <b>" + EscapeHtml(query) +
                    "</b></p>\n</body></html>");
  } else {
    // show number of results found
    if (results.size() == 1) {
      ret.AppendToBody("<p>1 result found for <b>" + EscapeHtml(query) +
                     "</b></p>\n");
    } else {
      ret.AppendToBody("<p>" + EscapeHtml(std::to_string(results.size())) +
                     " results found for <b>" + EscapeHtml(query) +
                     "</b></p>\n");
    }

    // show each file from query results, hyperlinking it
    ret.AppendToBody("<ul>\r\n");
    for (uint32_t i = 0; i < results.size(); i++) {
      ret.AppendToBody(" <li> <a href=\"");

      string file_name = results[i].document_name;

      // add static to non-http links
      if (file_name.substr(0, 7) != "http://") {
        results[i].document_name.replace(0, 12, "/static/");
      }

      ret.AppendToBody(EscapeHtml(results[i].document_name) + "\">" +
                       EscapeHtml(file_name) + "</a> [" +
