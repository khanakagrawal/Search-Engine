...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  // start where the doc table and index start
  if (fseek(f, sizeof(IndexFileHeader), SEEK_SET) != 0) {
    return kFailedWrite;
  }

  int size = doctable_bytes + memidx_bytes;
  char read;
  // iterate through each char, folding it into checksum
  for (int i = 0; i < size; i++) {
    if (fread(&read, sizeof(char), 1, f) == 1) {
      crc.FoldByteIntoCRC(read);
    } else {
      return kFailedWrite;
    }
  }
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    // STEP 4.
    int num_elts = LinkedList_NumElements(ht->buckets[i]);
    int r = WriteHTBucketRecord(f, record_pos, num_elts, bucket_pos);
    if (r != sizeof(BucketRecord)) {
      return kFailedWrite;
    }
    record_pos += r;
    if (num_elts != 0) {
      int b = WriteHTBucket(f, bucket_pos, ht->buckets[i], fn);
      bucket_pos += b;
    }
  }
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    DocPositionOffset_t cur_position = static_cast<DocPositionOffset_t>
                                       (reinterpret_cast<uintptr_t>(payload));
    position = DocIDElementPosition(cur_position);
...
static int WriteWordToPostingsFn(FILE* f,
  ...
  // STEP 19.
  // Calculate and return the total amount of data written.
  return sizeof(WordPostingsHeader) + ht_bytes + word_bytes;
}
}  // namespace hw3
